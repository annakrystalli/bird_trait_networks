...
meta.var_columns <- columns[columns %in% paste(fcodes, "file.name", sep = "_")]
...
ui = fluidPage(
  ...
  actionButton("BUTupdateD", "Update data_log"),
  h3("current data_log"),
  ...
  h1("Configure vnames"),
  actionButton("BUTupdateV", "Update vnames"),
  h3("current vnames"),
  br(),
  ...
  bsModal("modalnew2", "Edit vnames entry", "BUTnew2", size = "large",
          selectInput("csv.file2", label = "Select vnames column to edit", choices = vname.cols, selected = vname.cols[1], 
                      multiple = FALSE,
                      selectize = TRUE, width = NULL, size = NULL),
          selectInput("encodingIN", label = "select input file encoding", choices = c("", iconvlist()), selected = "", 
                      multiple = FALSE,
                      selectize = TRUE, width = NULL, size = NULL),
...
server = function(input, output, session) {
  ...
  observeEvent(input$BUTupdateV, {
    ...
    if(!identical(v$vnames, update_vnames())){
      ...
      v$msg2 <- "vnames updated from FS. Unsaved changes in browser vnames"
      v$vnames <- update_vnames()
