...
read_chunk(paste0(script.folder, "Phylonetworker_setup.R"))
source(paste0(script.folder, "Phylonetworker_functions.R"))
...
Next initialise the analysis by calling **`project_ui`**. This script calls a number of setup scripts (for details see [**`project_ui_README`**]()). It also sources the previously defined:
...
The phylocors function calculates **phylogenetically corrected correlations** between pairs of traits (defined by the *character vector of the trait names* supplied to argument `pair`), which are then used as edge weights in the network. The method is determined by the argument `pair.types`. 
  ...
  for more details see [**`pglsPhyloCor` README**]()
```{r}
