...
The phylocors function calculates **phylogenetically corrected correlations** between pairs of traits (defined by the *character vector of the trait names* supplied to argument `pair`), which are then used as edge weights in the network. The method is determined by the argument `pair.types`. If the analysis has already been run for a particular pair trait, an output has been saved in **`script.folder` data/phy_models/** (files too large so currently I keep the data locally) and argument **`overwite = F`**, the function just loads model and extracts data.
